核心逻辑完成后，我们的注册流程还存在几个显著的问题需要优化：
- 接口传来的值没有任何验证，用户名、密码、邮箱都应该必传，且需要有一定的安全验证，比如密码应该在`6-12`位之间，邮箱应该是`xx@xx.xx`格式；
- 禁止注册相同的用户名；
- 密码不应该明文入库，需要加密；
...
## 参数校检
`GoFrame`内置了强大的数据校检功能，只需要在结构体字段`tag`上加上`v`即可启用。
...
```
`The Username field is required`是一句英文提示，需要改成更友好的中文提示。
### 参数校检i18n
从[Github](https://github.com/gogf/gf/blob/master/util/gvalid/testdata/i18n/cn/validation.toml)下载文件并且存放到`i8n`目录，直接从下文复制也行。
...
```
只有代码上的检测还不够安全，在实际业务中，我们还会加上数据表唯一索引：
```sql
...
## 密码加密
数据表中的密码以明文保存，是一种非常不安全的行为，在实际业务开发中，一般会对明文密码进行非对称加密，例如`md5`。
...
## Register 函数优化
在`model`层自定义一个数据模型，用以保存所有的用户入参。
