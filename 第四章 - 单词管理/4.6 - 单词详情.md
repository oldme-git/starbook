单词详情同样使用`GET`方式，用作获取单词的详细信息，会包含例句，中文翻译等字段。
## 添加Api
---
...
`words/{id}`是一种路由绑定方式，它会识别类似`words/1`，`words/2`，`words/abc`这种路由，并将匹配到的值赋予`Id`字段。`Id`字段的类型是`uint`，如果值不合法，则会使用`uint`的零值。比如访问`words/abc`，`Id`字段的值就会是`0`。
## 编写Logic
---
