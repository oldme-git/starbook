本章将正式开发**星辰英语本**项目，我们将开发第一个接口：用户注册。
...
`GoFrame`通过 [gdb](https://goframe.org/docs/core/gdb) 组件内置了`ORM`功能，提供便捷增删改查的操作。 
...
`ORM`仅仅提供了数据表映射功能，实际执行`SQL`语句还需要用到数据库驱动。
...
### 检查驱动
完成后，我们需要检查一下数据库驱动是否安装成功。在配置文件中加入数据库配置。如果配置文件里面有内容则全部清除，保持和下面的一致：
