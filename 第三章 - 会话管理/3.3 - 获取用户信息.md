...
`InfoRes`结构体中保存着接口的响应数据，这里响应了四个字段，其中`*gtime.Time`数据类型是`gtime`组件提供的框架时间类型。
## 编写Logic
---
...
在`Logic`中不能直接获取`HTTP`对象，需要使用`g.RequestFromCtx(ctx).Request`来获取。获取`Token`后解析它，拿到用户`ID`后调用`GoFrame ORM`从库里读取用户信息。`Scan` 方法将查询结果转换为结构体或者结构体数组， 会根据给定的参数类型自动识别执行的转换类型，这是数据查询操作的常客。
## Controller调用Logic
---
...
## 注册新的控制器
---
分组路由注册新的控制器时，使用`group.Middleware`将`Auth`中间件注入，凡是在本组下的控制器在访问前都需要经过认证。
