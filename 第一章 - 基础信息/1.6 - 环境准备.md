...
### 接口测试工具
开发接口完成后，我会使用`curl`命令测试它，用于测试我们开发的接口是否符合预期。写命令总是不方便的，所以推荐你准备一个接口测试工具。比较流行的接口测试工具有`Postman`、`Apifox`、`Apipost`等，它们大同小异，你可以根据自己的喜好选择一个。
...
### 升级项目
默认安装的`GoFrame`不一定是最新版本，你可以执行一下命令升级：
```bash
$ gf up
```
...
如果你对目录结构不太了解，看起来一头雾水，不必担心，重要的目录都会在后续用到。

然后我们将初始的示例文件全部删除，留下一个空白的环境，用作后续开发。先按`CTRL+C`终止项目运行，删除如下目录内的所有文件：
```text
api/*
internal/controller/*
```
